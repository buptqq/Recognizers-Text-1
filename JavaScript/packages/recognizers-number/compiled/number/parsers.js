...
bignumber_js_1.BigNumber.config({ ERRORS: false });
// The exponent value(s) at which toString returns exponential notation.
bignumber_js_1.BigNumber.config({ EXPONENTIAL_AT: [-5, 15] });
class BaseNumberParser {
    constructor(config) {
        ...
        this.textNumberRegex = recognizers_text_1.RegExpUtility.getSafeRegExp(String.raw `(?=\b)(${singleIntFrac})(?=\b)`, "gis");
        this.arabicNumberRegex = recognizers_text_1.RegExpUtility.getSafeRegExp(String.raw `\d+`, "is");
    ...
    digitNumberParse(extResult) {
        ...
        let matches = recognizers_text_1.RegExpUtility.getMatches(this.config.digitalNumberRegex, handle);
        if (matches) {
    ...
    fracLikeNumberParse(extResult) {
        ...
        if (resultText.includes(this.config.fractionMarkerToken)) {
            ...
            if (mixedIndex !== fracWords.length && numerValue < denomiValue) {
                ...
                result.value = new bignumber_js_1.BigNumber(intValue).plus(new bignumber_js_1.BigNumber(numerValue).dividedBy(denomiValue));
            }
            else {
                // (intValue + numerValue) / denomiValue
                result.value = new bignumber_js_1.BigNumber(intValue + numerValue).dividedBy(denomiValue);
            }
        }
        return result;
    }
    ...
    powerNumberParse(extResult) {
        ...
        let scale = new bignumber_js_1.BigNumber(10);
        let dot = false;
        let isLessZero = false;
        let tmp = new bignumber_js_1.BigNumber(0);
        for (let i = 0; i < handle.length; i++) {
            ...
            if (ch === '^' || ch === 'E') {
                ...
                tmp = new bignumber_js_1.BigNumber(0);
                scale = new bignumber_js_1.BigNumber(10);
                dot = false;
                isLessZero = false;
            }
                ...
                dot = true;
                scale = new bignumber_js_1.BigNumber(0.1);
            }
            else if (ch === '-') {
    ...
    getPointValue(matchStrs) {
        ...
        if (this.config.cardinalNumberMap.has(firstMatch) && this.config.cardinalNumberMap.get(firstMatch) >= 10) {
            ...
            let scale = new bignumber_js_1.BigNumber(0.1);
    ...
    getDigitalValue(digitStr, power) {
        ...
        let calResult = new bignumber_js_1.BigNumber(0);
        if (isFrac) {
