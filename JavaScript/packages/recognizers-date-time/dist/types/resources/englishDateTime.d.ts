export declare namespace EnglishDateTime {
    ...
    const RelativeRegex = "(?<order>next|upcoming|this|last|past|previous|current|the|my)";
    const StrictRelativeRegex = "(?<order>next|upcoming|this|last|past|previous|current)";
    const NextPrefixRegex = "(next|upcoming)\\b";
    ...
    const PeriodYearRegex = "\\b(?<year>19\\d{2}|20\\d{2})\\b";
    ...
    const TimeUnitRegex = "(?<unit>hours|hour|hrs|hr|h|minutes|minute|mins|min|seconds|second|secs|sec)\\b";
    ...
    const BeforeRegex = "\\b(before|in advance of|prior to)\\b";
    const AfterRegex = "\\b(after)\\b";
    const SinceRegex = "\\b(since)\\b";
    const AgoRegex = "\\b(ago)\\b";
    const LaterRegex = "\\b(later|from now)\\b";
    const InConnectorRegex = "\\b(in)\\b";
    const AmPmDescRegex = "(ampm)";
    const MorningStartEndRegex = "(^(morning))|((morning)$)";
    const AfternoonStartEndRegex = "(^(afternoon))|((afternoon)$)";
    ...
    const WeekDayAndDayOfMothRegex: string;
